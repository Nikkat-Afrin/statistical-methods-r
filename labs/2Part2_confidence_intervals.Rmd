...
```{r bar-plot-pop-samp, fig.height=2.5, fig.width=10}
...
```{r summ-stat-pop-samp, results = TRUE}
samp %>%
  count(climate_change_affects) %>%
  mutate(p = n /sum(n))
```

**In our calculation the proportion of the adults who believe that the climate change effect them is 63% and the adults who believe that the climate change doesn't effect them is 36.7% **
...
**A 95% confidence interval means that if we were to take many random samples and calculate a confidence interval from each sample, 95% of those intervals would contain the true population proportion. the below graph shown in the class gives a clear understanding about this interpretation**

```{r grapg_1}
knitr::include_graphics("C:/Users/nikka/OneDrive/Desktop/R notes/05_foundations_for_inference/11_foundations_for_inference/graph_95.png")
```
...
**Yes my confidence interval captures approximately the true population proportion of US adults who think climate change affects their local community**
...
**As we are calculating 95% confidence intervals, we expect that 95% of the intervals constructed by different students will capture the true population proportion.**
...
```{r graph_2}
knitr::include_graphics("C:/Users/nikka/OneDrive/Desktop/R notes/graph_2.png")
```

**The plot shows 50 confidence intervals, each represented by a horizontal line, with blue lines indicating that the confidence interval includes the true population proportion and red lines indicating that the confidence interval does not include it.**
                            **Thus, the proportion is = 47/50 = 0.94**
                **By, this we can say that 94% of the intervals have the true population value**

**As, the population proportion we have given around is 95% but we got 94% the proportion is not exactly equal to the confidence level. This is happening because the sampling is done randomly and by replacement. So, the proportion hovers around 95% but not exactly in this case**
...
```{r graph_3}
knitr::include_graphics("C:/Users/nikka/OneDrive/Desktop/R notes/graph_3.png")
```

**As we can see in the above graph that this time i took the confidence interval as 99%. If we compare it with the previous interval we can see that the confidence interval got wider, we can observe that in the x-axis scale. If we increase the confidence interval it becomes wider because it captures all the possible values and give the most number of true population proportion. As we reduce the confidence interval the interval becomes narrower as it doesn't capture most number of true population proportion**
...
```{r confidence interval infer modified}
set.seed(2222)
samp1 <- samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%  
  generate(reps = 1000, type = "bootstrap") %>%                    
...
**In the above code i have set the confidence level to 99%, by running this we can be 99% confident that the true proportion of U.S. adults who believe climate change is affecting their local community lies between 0467 (46.7%) and 0.767 (76.7%). By this we can infer that as the confidence level increases the interval size widens.**
...
**In this question I want to take confidence level as 90, as I am decreasing the confidence level the width of this interval narrows down compared to the previous intervals**

```{r confidence interval infer modified2}
set.seed(2223)
samp2 <- samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%  
  generate(reps = 1000, type = "bootstrap") %>%                    
...
**In the above code i have set the confidence level to 90%, by running this we can be 90% confident that the true proportion of U.S. adults who believe climate change is affecting their local community lies between 0.533 (53.3%) and 0.733 (73.3%). By this we can infer that as the confidence level increases the interval size narrows**

```{r graph_4}
knitr::include_graphics("C:/Users/nikka/OneDrive/Desktop/R notes/graph_4.png")
```

**The plot shows 50 confidence intervals, each represented by a horizontal line, with blue lines indicating that the confidence interval includes the true population proportion and red lines indicating that the confidence interval does not include it.**
                            **Thus, the proportion is = 44/50 = 0.88**
                **By, this we can say that 88% of the intervals have the true population value**

**As, the population proportion we have given around is 90% but we got 88% the proportion is not exactly equal to the confidence level. This is happening because the sampling is done randomly and by replacement. So, the proportion hovers around 90% but not exactly in this case**
...
**If we increase the confidence interval it becomes wider because it captures all the possible values and give the most number of true population proportion. As we reduce the confidence interval the interval becomes narrower as it doesn't capture most number of true population proportion**
...
**The number of bootstrap samples does not affect the standard error or the width of the confidence interval.**
